 ...
 * @param {object} schema - { title, class, source, article, skill, paragon, date }
 */
...
export function createDbRowSingle(client, databaseId, props = {}, schema = {}){
  ...
  const properties = {};
  properties[S.title] = { title: [{ type:'text', text:{ content: props.title || '(제목 없음)'} }]};
  if (props.className) properties[S.class] = { select: { name: props.className } };
  ...
  return client.post('pages', { parent: { database_id: databaseId }, properties });
}

/** (옵션) 부모 페이지 아래 자식 페이지를 만들고 싶을 때 */
